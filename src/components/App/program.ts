...
export const programExecutorFactory = ({
    ...
    return function execProgramStep() {
        ...
        console.log('execProgramStep, program before command:');
        console.log(JSON.stringify(pr));

        const debounce = (callback: Function) => setTimeout(() => {
            callback();
            nextStepCallback();
        }, programDelayMS);
        ...
        const programIsOver = pr.find(({value}: { value: number }) => value > 0) === undefined;
